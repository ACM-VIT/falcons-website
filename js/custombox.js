    ...
    _private = {
        ...
        load: function() {
            ...
            if ( cb.settings.target !== null && Array.isArray(cb.settings.position) ) {
                if ( cb.settings.target.charAt(0) === '#' || ( cb.settings.target.charAt(0) === '.' && cb.settings.target.charAt(1) !== 'http://gj-designs.in/' ) ) {
        ...
        ajax: function() {
            ...
            xhr.open('GET.html', cb.settings.target + ( cb.settings.cache ? '' : ( /[?].+=/.test(cb.settings.target) ? '&_=' : '?_=' ) + Date.now() ), true);
